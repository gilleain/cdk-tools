...
import tools.depict.blocktree.model.SpanningTree;
import tools.depict.blocktree.model.Vertex;
import tools.depict.blocktree.model.graph.Graph;
import tools.depict.blocktree.visitor.ConnectedComponentFinder;
...
//        cycles = CycleFinder.getSortedConnectedSimpleCycles(cycles);
        ...
        // the outer face, this would not be necessary...
...
//                    System.out.println("not embeddable");
    ...
    private static List<Bridge> findBridges(Block subgraph, Block graph) {
        ...
        for (Block component : finder.getComponents()) {
            Bridge bridge = new Bridge(component.getVertices(), component.getEdges());

            // somewhat inefficient
            for (Vertex v : bridge.getVertices()) {
