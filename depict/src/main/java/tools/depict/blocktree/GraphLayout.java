...
import tools.depict.blocktree.embedder.GraphEmbedder;
import tools.depict.blocktree.embedder.GraphEmbedding;
...
import tools.depict.layout.ILayout;

public class GraphLayout implements ILayout<IAtomContainer> {
	...
	public void layout(IAtomContainer atomContainer) {
    	GraphEmbedding embedding = GraphEmbedder.embed(atomContainer);
		// TODO CANVAS!!
    	Representation rep = layout(embedding, new Rectangle2D.Double(0, 0, 100, 100));
    	List<Point2D> points = rep.getPoints();
    	for (int index = 0; index < points.size(); index++) {
    		Point2D p = points.get(index);
    		atomContainer.getAtom(index).setPoint2d(new Point2d(p.getX(), p.getY()));
    	}
	}
    ...
    private void layout(GraphEmbedding embedding, Representation rep,
        ...
        if (isTreePart) {
            ...
            if (articulationVertex == null) {
                rep.add(layout.layout(g, partCenter));
            } else {
